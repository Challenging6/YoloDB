...
def letterbox_label(x, w=640, h=640, padw=0, padh=0):
...
def build_processors(process_list):
    processors = []
    #print(process_list)
    for process in process_list:
        class_name = process.pop('class')
       # print(**process)
        processor = eval(class_name)(**process)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, image_weights=False,
                 cache_images=False, stride=32, pad=0.0, prefix='', process_list=[]):
        self.img_size = img_size
        ...
        self.processors = build_processors(process_list) 
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        ...
                        for i in range(len(lines)):
                            label = lines[i][-1]
                            line  = [k.strip('\ufeff').strip('\xef\xbb\xbf') for k in lines[i]]
                            poly = np.array(list(map(float, line[:8]))).reshape((-1, 2))
                            item = {}
                            poly[:, 0] = poly[:, 0]/shape[0]
                            poly[:, 1] = poly[:, 1]/shape[1]
                            item['poly'] = poly
                            item['text'] = label
                            lines[i] = item  
                    ...
                    if len(lines):
                        for item in lines:
                           # print(item['poly'])
                            assert item['poly'].shape[0] == 4, 'labels require 4 point'
                            assert (item['poly'] >= 0).all(), 'negative labels'
